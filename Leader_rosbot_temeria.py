...
class OdometryServer(Node):
    ...
    def calculate_movement(self, current_position, current_orientation):
        if current_position is None or current_orientation is None:
            return {
                'message': "Position or orientation data is unavailable.",
                'distance': 0,
                'rotation': 0,
        ...
        if self.last_position and self.last_orientation:
            ...
            return {
                'message': "Initial position and orientation set.",
                'distance': 0,
                'rotation': 0,
                'total_distance': self.total_distance,
                'twist': {
    ...
    def odom_callback(self, msg):
        ...
        if self.conn:
            try:
                ...
                movement_message = self.calculate_movement(current_position, current_orientation)
                self.send_movement_data(movement_message)
    ...
    def send_movement_data(self, movement_message):
        try:
            if isinstance(movement_message, dict):
                serialized_message = pickle.dumps(movement_message)
                message_length = len(serialized_message).to_bytes(4, byteorder='big')
                self.conn.sendall(message_length + serialized_message)
                self.get_logger().info(f"Sent movement description: {movement_message}")
            else:
                self.get_logger().error(f"Expected a dict, but got {type(movement_message)}: {movement_message}")
        except Exception as e:
    ...
    def timer_callback(self):
        if self.conn:
            self.get_logger().info("Logging movement data")
            self.odom_callback(None)  # Trigger logging
